...
pub fn part_1(input: &str) -> u32 {
    let (gamma_rate_bits, epsilon_rate_bits) = input
        .lines()
        .map(|s| s.chars().map(|c| if c == '1' { 1 } else { -1 }).collect::<Vec<i32>>())
        .reduce(|acc, values| acc.iter().zip(values.iter()).map(|(a, b)| a + b).collect::<Vec<i32>>())
        .unwrap()
        .iter()
    ...
    let epsilon_rate = u32::from_str_radix(&epsilon_rate_bits.iter().collect::<String>(), 2).unwrap();
...
impl OxygenGeneratorRating {
    ...
    fn next(self) -> OxygenGeneratorRating {
        let (ones, zeros): (Vec<&str>, Vec<&str>) = self.tails.iter().map(|s| &s[..]).partition(|s| s.starts_with("1"));
